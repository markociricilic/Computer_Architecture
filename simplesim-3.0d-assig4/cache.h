...
#define MAX_NUM_DELTAS 16       // Number of deltas stored per entry
#define TABLE_SIZE 128          // Number of entries in the delta table
#define MAX_PREFETCH 10         // Balancing number of prefetches issued at a time to reduce overhead and improve cache utilisation

// Format of a delta correlating prediction table entry from the paper (page 5)
struct dcpt_entry {
	md_addr_t pc;
	md_addr_t last_addr;
	md_addr_t last_prefetch;
	int delta[MAX_NUM_DELTAS];
	int delta_ptr
};
...
struct cache_t
{
  ...
  /* ECE552 Assignment 4 - BEGIN CODE*/
  int dcpt_size;            // delta table size
	struct dcpt_entry *dcpt;  // delta table instantiation
  /* ECE552 Assignment 4 - END CODE*/
